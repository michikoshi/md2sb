...
function iterateTokenRaw(iterator, level, token) {
    ...
    if (token.children) {
        return token.children.map((child) => iterateTokenRaw.bind(null, iterator, level + 1, child));
    }
    return CONTINUE;
}
...
function getTemplate(parent) {
    ...
    let onlyEntry = true;
    let base;
    while ((parent.type === "div" || parent.type === undefined) &&
        parent.children &&
                ...
                if (base.children.length > 0 && base.children[0].children) {
                    userTitle =
                        base.children
                            .map((token) => {
                            const max = 15;
                            let text = (0, toScrapbox_1.toSimpleText)(token).replace(/[\t\n]/gi, "");
                            if (text.length > max) {
...
const guessTitle = (parsed, sb, evaluateTitle) => {
